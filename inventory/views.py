...
def stock_movement_add(request):
    ...
    if not accessible_warehouses:
        messages.error(request, "Sie haben keine Berechtigung, Bestandsbewegungen durchzuführen.")
        return redirect('stock_movement_list')

    # Vorausgewähltes Produkt, Bewegungstyp und Lager (z.B. aus URL-Parametern)
    preselected_product_id = request.GET.get('product', None)
    preselected_type = request.GET.get('type', None)
    preselected_warehouse_id = request.GET.get('warehouse', None)

    preselected_product = None
    preselected_warehouse = None

    if preselected_product_id:
        preselected_product = get_object_or_404(Product, pk=preselected_product_id)

    if preselected_warehouse_id:
        warehouse = get_object_or_404(Warehouse, pk=preselected_warehouse_id)
        if user_has_warehouse_access(request.user, warehouse, 'manage_stock'):
            preselected_warehouse = warehouse
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Überprüfen Sie, ob der Benutzer Zugriff auf das ausgewählte Lager hat
            if not user_has_warehouse_access(request.user, movement.warehouse, 'manage_stock'):
            ...
            # Produkt und aktueller Bestand im Lager
            product = movement.product
            # Bestand im spezifischen Lager abfragen
            try:
                product_warehouse = ProductWarehouse.objects.get(product=product, warehouse=movement.warehouse)
                current_stock = product_warehouse.quantity
            except ProductWarehouse.DoesNotExist:
                current_stock = 0

            # Validieren: Bei Abgang darf Bestand nicht negativ werden
            if movement.movement_type == 'out' and movement.quantity > current_stock:
                messages.error(
                    request,
                    f'Fehler: Die Abgangsmenge ({movement.quantity}) überschreitet den aktuellen Bestand ({current_stock}) im Lager {movement.warehouse.name}.'
                )
                return render(request, 'inventory/stock_movement_form.html', {
                    'form': form,
                    'preselected_product': preselected_product,
            ...
            try:
                # Bestand im spezifischen Lager aktualisieren
                update_product_stock(
                    ...
                    movement.reference,
                    movement.notes,
                    movement.warehouse.id  # Lager-ID hinzufügen
                )

                messages.success(
                    request,
                    f'Bestandsbewegung im Lager {movement.warehouse.name} wurde erfolgreich erfasst.'
                )

                # Zurück zur Produktdetailseite, wenn ein Produkt vorausgewählt war
                if preselected_product:
                    return redirect('product_detail', pk=preselected_product.id)
                else:
                    return redirect('stock_movement_list')
        ...
        if preselected_type:
            initial_data['movement_type'] = preselected_type
        if preselected_warehouse:
            initial_data['warehouse'] = preselected_warehouse.id
...
def product_transfer(request, product_id):
    """Produkt zwischen Lagern transferieren."""
    product = get_object_or_404(Product, pk=product_id)

    # Nur Lager anzeigen, auf die der Benutzer Zugriff hat (für Eingang und Ausgang)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Prüfen, ob der Benutzer Zugriff auf beide Lager hat
            if not (user_has_warehouse_access(request.user, source_warehouse, 'manage_stock') and
                ...
                return redirect('product_transfer', product_id=product_id)

            # Prüfen, ob genügend Bestand im Quelllager vorhanden ist
            try:
                source_product_warehouse = ProductWarehouse.objects.get(product=product, warehouse=source_warehouse)
                if source_product_warehouse.quantity < quantity:
                    messages.error(request,
                                   f"Nicht genügend Bestand im Quelllager. Verfügbar: {source_product_warehouse.quantity}")
                    return redirect('product_transfer', product_id=product_id)
            except ProductWarehouse.DoesNotExist:
                messages.error(request, "Kein Bestand im Quelllager vorhanden.")
                return redirect('product_transfer', product_id=product_id)

            # Transaktion für den Transfer
            with transaction.atomic():
                # Abgang vom Quelllager
                StockMovement.objects.create(
                    product=product,
                    quantity=quantity,
                    movement_type='out',
                    reference=f'Transfer zu {destination_warehouse.name}',
                    notes=notes,
                    created_by=request.user,
                    warehouse=source_warehouse
                )

                # Bestand im Quelllager aktualisieren
                source_product_warehouse.quantity -= quantity
                source_product_warehouse.save()

                # Zugang zum Ziellager
                StockMovement.objects.create(
                    product=product,
                    quantity=quantity,
                    movement_type='in',
                    reference=f'Transfer von {source_warehouse.name}',
                    notes=notes,
                    created_by=request.user,
                    warehouse=destination_warehouse
                )

                # Bestand im Ziellager aktualisieren
                dest_product_warehouse, created = ProductWarehouse.objects.get_or_create(
                    product=product,
                    warehouse=destination_warehouse,
                    defaults={'quantity': 0}
                )
                dest_product_warehouse.quantity += quantity
                dest_product_warehouse.save()

            messages.success(request,
                             f"{quantity} Einheiten von {product.name} wurden erfolgreich von {source_warehouse.name} nach {destination_warehouse.name} transferiert.")
            return redirect('product_warehouses', product_id=product_id)
    ...
    return render(request, 'inventory/product_transfer.html', context)


@login_required
@permission_required('inventory', 'edit')
def product_adjust_stock(request, product_id, warehouse_id):
    """Bestand eines Produkts in einem bestimmten Lager korrigieren."""
    product = get_object_or_404(Product, pk=product_id)
    warehouse = get_object_or_404(Warehouse, pk=warehouse_id)

    # Zugriffsrechte prüfen
    if not user_has_warehouse_access(request.user, warehouse, 'manage_stock'):
        return HttpResponseForbidden("Sie haben keine Berechtigung, den Bestand in diesem Lager zu korrigieren.")
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Transaktion für die Bestandskorrektur
            with transaction.atomic():
                ...
                StockMovement.objects.create(
                    ...
                    movement_type='adj',
                    reference='Bestandskorrektur',
                    notes=f'Bestand korrigiert von {current_stock} auf {new_quantity}. Grund: {reason}',
                    created_by=request.user,
                    warehouse=warehouse
                )
            ...
            messages.success(request,
                             f"Bestand von {product.name} in {warehouse.name} wurde erfolgreich auf {new_quantity} aktualisiert.")
            return redirect('product_warehouses', product_id=product_id)

        except ValueError as e:
            messages.error(request, f"Fehler bei der Bestandskorrektur: {str(e)}")
            return redirect('product_adjust_stock', product_id=product_id, warehouse_id=warehouse_id)
    ...
    return render(request, 'inventory/product_adjust_stock.html', context)


# In inventory/forms.py

class WarehouseForm(forms.ModelForm):
    """Form für die Lagerverwaltung."""
